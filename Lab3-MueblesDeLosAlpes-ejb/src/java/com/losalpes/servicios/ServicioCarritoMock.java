...
public class ServicioCarritoMock implements IServicioCarritoMockRemote, IServicioCarritoMockLocal {
    ...
    public void comprar(Usuario usuario) {
        ...
        for (int j = 0; j < usuario.getCompras().size(); j++) {
            try {
                RegistroVenta compra1 = new RegistroVenta(new Date(System.currentTimeMillis()), usuario.getCompras().get(j).getProducto(), usuario.getCompras().get(j).getCantidad(), null, usuario);
                persistencia.create(compra1);
